...
SERVER_KEY=$(op document get "shaw_service_server_prod_key" --vault world_site | base64 -w 0)

# check if values are retrieved successfully
if [[ -z "$SERVER_CERT" || -z "$SERVER_KEY" ]]; then
  echo "Error: failed to get identity prod server certificate or key from 1Password."
  exit 1
fi
